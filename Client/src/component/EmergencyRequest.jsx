...
const EmergencyRequest = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.emergencyType) {
      setMessage({ type: 'error', text: '⚠️ Please select an emergency type' });
      setIsSubmitting(false);
      return;
    }

    if (!formData.description) {
      setMessage({ type: 'error', text: '⚠️ Please add a description' });
      setIsSubmitting(false);
      return;
    }

    if (!formData.location) {
      setMessage({ type: 'error', text: '⚠️ Please provide a location' });
      setIsSubmitting(false);
      return;
    }
